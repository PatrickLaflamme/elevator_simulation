...
def test_existing_stop_assignment_strategy_prefers_elevator_already_stopping_at_source_and_dest():
    ...
    assignment = ExistingStopStrategy().assign_elevator(
...
def test_existing_stop_assignment_strategy_prefers_elevator_already_stopping_at_source_over_none():
    elevators = [Elevator(num_floors=10, max_capacity=10) for _ in range(3)]
    # first elevator is going to stop at floors 3 and 6
    elevators[0].assign(Passenger(id="", source_floor=4, destination_floor=6, request_time=0))
    # second elevator is going to stop at floors 4 and 7
...
def test_existing_stop_assignment_strategy_selects_elevator_able_to_pick_up_soonest():
    elevators = [Elevator(num_floors=10, max_capacity=10) for _ in range(3)]
    # first elevator is going to stop at floors 3 and 6
    elevators[0].assign(Passenger(id="", source_floor=4, destination_floor=6, request_time=0))
    # second elevator is going to stop at floors 4 and 7
